...
(function(global, factory) {
  ...
  /**
   * @options
   * @param onZoomCallback [function] <$.noop> ""
   ...
   * @param controls.zoomLabel [string] <> "Custom zoom label selecter"
  ...
  var properties = {
    ...
    // How much more zoom beyond 100% to allow
    // 1 means max of 100%; 2 means max of 200%; 8 means max of 800%; 16 means max of 1600%
    overZoom: 16,
    clickModify: 'move',

    // Whether we are current at min or max zoom limits
    atMinZoom: false,
    atMaxZoom: true,
    ...
    destroy: function() {
      var $targets = $(this).each(function(i, target) {
        ...
        if (data) {
          ...
          data.controls.$zoomLabel.off(".zoomer");
  ...
  function _build($target, data) {
    if (!$target.data("zoomer")) {
      ...
      if (data.controls.zoomIn || data.controls.zoomOut || data.controls.zoomLabel || data.controls.next || data.controls.previous) {
        data.controls.$zoomIn = $(data.controls.zoomIn);
        data.controls.$zoomOut = $(data.controls.zoomOut);
        data.controls.$zoomLabel = $(data.controls.zoomLabel);
        ...
        html += '<span class="zoomer-zoom-label">100%</span>';
        ...
        data.controls.$zoomLabel = data.$zoomer.find(".zoomer-zoom-label");
      }
      ...
      data.controls.$zoomOut.on("touchstart.zoomer mousedown.zoomer", data, _zoomOut)
        .on("touchend.zoomer mouseup.zoomer", data, _clearZoom);
      data.controls.$zoomLabel.on("touchstart.zoomer mousedown.zoomer", data, _zoomLabel)
        .on("touchend.zoomer mouseup.zoomer", data, _clearZoom);
  ...
  function _onImageLoad(e) {
    ...
    data.$zoomer.removeClass('loading');
    ...
    data.targetImageHeight = data.minHeight = data.naturalHeight;
    data.targetImageWidth  = data.minWidth  = data.naturalWidth;
    data.maxHeight = data.naturalHeight * data.overZoom;
    data.maxWidth = data.naturalWidth * data.overZoom;
  ...
  function _render() {
    for (var i = 0, count = $instances.length; i < count; i++) {
      ...
      if (typeof data === "object") {
        ...
        // Update zoom label
        var zoomLevel = Math.ceil(scaleX*100) || 100;
        var zoomLabel = zoomLevel + '%';
        if (data.maxZoomLabel && zoomLevel === data.overZoom*100) {
          zoomLabel = data.maxZoomLabel;
        }
        data.controls.$zoomLabel.html(zoomLabel);
        ...
        // Is it at min zoom?
        // We allow 3px of error buffer due to the way the width is calculated as we zoom continuously
        // e.g. We may get to zoom all the way out to only 887 when minWidth is 885
        if ((data.imageWidth - 3) <= data.minWidth) {
          data.atMinZoom = true;
        } else {
          data.atMinZoom = false;
        }

        // Is it at max zoom?
        if (zoomLevel === data.overZoom*100) {
          data.atMaxZoom = true;
        } else {
          data.atMaxZoom = false;
        }

        // Run callback function
        if (data.onZoomCallback) {
          data.onZoomCallback.apply(data.$zoomer, [
            data
          ]);
        }
   ...
   * @method private
   * @name _zoomLabel
   * @description Handles zoom label button click
   * @param e [object] "Event Data"
   */
  function _zoomLabel(e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  function _dragStart(e) {
    ...
    if (data.clickModify == "zoomIn") {
      $window.on("mouseup.zoomer", data, _clearZoom);
      _zoomIn(e);
      return;
    } else if (data.clickModify == "zoomOut") {
      $window.on("mouseup.zoomer", data, _clearZoom);
      _zoomOut(e);
      return;
    }
  ...
  function _getTransform3DSupport() {
    ...
    /* If we use translated3d, pixelated image-rendering mode will NOT be supported */
  ...
  var jQueryZoomer = function(method) {
    if (pub[method]) {
  ...
  window.zoomer = function() {
    jQueryZoomer.apply(arguments[0], Array.prototype.slice.call(arguments, 1));
  };
}));